...
Matrix createMatrix(int rows, int cols) {
    ...
    for (i = 0; i < rows; i++) {
        ...
        if (matrix.mat[i] == NULL) {
            ...
            /* Freeing the previously allocated rows in case of failure */
            for (k = 0; k < i; k++) {
                free(matrix.mat[k]);
            }
...
/* Creating an empty matrix of size: rows x cols */
Matrix readMatrix(char filename[]){
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (current_cols > cols) {
            cols = current_cols;/* Updating the maximum columns found */
        }
    }
...
Matrix multiplyMatrices(Matrix A, Matrix B) {
    ...
    /* Checking if multiplication is possible (A.cols must be equal to B.rows) */
    if (A.cols != B.rows) {
        fprintf(stderr, "An Error Has Occurred");
        exit(EXIT_FAILURE);
    }
    
    /* Creating the multiplication result matrix, C, with the dimensions: A.rows x B.cols */
...
Matrix divisionMatrices_ij(Matrix A, Matrix B){
    ...
    /* Checking if division is possible (B must be square and invertible) */
...
Matrix sym(Matrix matrix){
    ...
    /* Looping through each pair of rows (data points) to compute the similarity */
    for (i = 0; i < n; i += 1) {
        for (j = 0; j < n; j += 1) {
...
Matrix ddg(Matrix matrix){
    ...
    /* Filling-in the diagonal of ddg with the sum of corresponding row elements from sym_mat */
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            if(i == j) {
                /* Setting the diagonal elements as the sum of the corresponding row in the similarity matrix */
                ddg.mat[i][j]= sum(sym_mat.mat[i], n);
            }
            else {
                /* Setting the Non-diagonal elements as 0 */
                ddg.mat[i][j] = 0;
            }
        }
    }
...
Matrix norm(Matrix matrix){
    ...
    /* Calculating the inverse square roots of the diagonal matrix */
    for (i = 0; i < D.rows; i += 1){
        for (j = 0; j < D.cols; j += 1){
            /* Only computing for diagonal elements */
            if(i == j)
            {
...
int main(int argc,char *argv[]) {
    ...
    /* Parsing the goal and file-name from command-line arguments */
    strcpy(goal, argv[1]);
    strcpy(filename, argv[2]);

    matrix = readMatrix(filename);

    /* Based on the goal, we compute and print the corresponding matrix */
