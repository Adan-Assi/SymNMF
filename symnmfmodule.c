...
/* Converting a Python list into a Matrix struct */
Matrix *listMatrixConverter(PyObject *data_list) {
    ...
    /* Checking the first row to determine the number of columns */
    PyObject *first_row = PyList_GetItem(data_list, 0);
    
    Py_ssize_t cols = PyList_Size(first_row);

    /* Creating the Matrix struct */
    ...
    *mat = createMatrix((int)rows, (int)cols);
    ...
    /* Filling the matrix with the Python list data */
    for (Py_ssize_t i = 0; i < rows; ++i) {
        PyObject *row = PyList_GetItem(data_list, i);
        if (!PyList_Check(row) || PyList_Size(row) != cols) {
            PyErr_SetString(PyExc_ValueError, "An Error Has Occurred");
            free(mat); /* Freeing in the case of an error */
            return NULL;
        }
        for (Py_ssize_t j = 0; j < cols; ++j) {
            ...
            mat->mat[i][j] = (float) PyFloat_AsDouble(item); /* Casting to float (for storage) */
        }
    }
    return mat;
}

/* Converting the Matrix struct to a Python list (2D list) */
static PyObject *matrixListConverter(Matrix result) {
    ...
    /* Creating the top-level list (for rows) */
    PyObject *result_list = PyList_New(rows);
    if (result_list == NULL) {
        return NULL;
    }

    /* Filling the Python list with values from the C matrix */
    for (int i = 0; i < rows; ++i) {
        /* Creating a list for each row */
        PyObject *row_list = PyList_New(cols);
        if (row_list == NULL) {
            Py_DECREF(result_list); /* Decrementing reference count to free memory for the top-level list if row creation fails */
        ...
        for (int j = 0; j < cols; ++j) {
            ...
            /* Converting each element to a Python float and setting it in the row list */
            ...
            /* No need to `DECREF` value as PyList_SetItem steals the reference */
            PyList_SetItem(row_list, j, value);
        }

        /* Adding the row to the main list */
...
static PyObject *Csymnmf(PyObject *self, PyObject *args){
    ...
    /* Parsing the Python argument to ensure we received a Python object */
    if (!PyArg_ParseTuple(args, "OO", &data_list_obj1,&data_list_obj2)) {
        return NULL;
    }

    /* Ensuring that the input is a 2D Python list */
    if (!PyList_Check(data_list_obj1)||(!PyList_Check(data_list_obj2))) {
        PyErr_SetString(PyExc_TypeError, "An Error Has Occurred");
        return NULL;
    }

    /* Converting the Python list to a Matrix struct */
    ...
    /* Calling the sym function, which operates on the Matrix struct */
    Matrix result = converge(*init_H,*W);

    free(init_H);
    free(W);

    /* Converting the result Matrix back to a Python 2D list */
...
static PyObject *Csym(PyObject *self, PyObject *args){
    ...
    free(converted_mat); /* Freeing the memory allocated for converted_mat */
...
static PyObject *Cddg(PyObject *self, PyObject *args){
    ...
    /* Ensuring the input is a 2D Python list */
    if (!PyList_Check(data_list_obj)) {
        PyErr_SetString(PyExc_TypeError, "An Error Has Occurred");
        return NULL;
    }
